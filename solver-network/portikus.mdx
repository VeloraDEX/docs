---
title: "Portikus solver network"
sidebarTitle: "Portikus"
...
Each app exposes its own surface for tracking an intent. Delta's is the order status you poll with `GET /v2/delta/orders/{orderId}`, which moves from submission to a terminal state like this:
...
<CardGroup cols={3}>
  ...
  <Card title="Your users never touch gas" icon="gas-pump">
    Settlement gas is handled inside the Portikus flow. The user signs an off-chain message and receives tokens.
  </Card>
