...
sidebarTitle: "Sealed-bid auctions"
...
flowchart TB
  ...
  G --> H[Protocol executes winning calldata]
  H --> I{Settled?}
  I -- "yes" --> J([Completed])
  I -. "no" .-> A
  F -. "no" .-> A
...
| Submit calldata that would deliver below the quote | The user's signed minimum is enforced by the settlement contract. A fill below it reverts on-chain. |
| Submit fill calldata without participating honestly | Only solvers registered in the Portikus registry contract can provide executable bids. A misbehaving solver can be removed, and removal is enforced on-chain. |
| Win with calldata that later reverts or becomes stale | The order re-enters the next auction round. The user's funds never moved, so the failed solver wasted only its own auction slot. |
...
- [Portikus Network](/solver-network/portikus) — the intent infrastructure behind these auctions.
