...
Any caller can get a partner ID for free just by passing `&partner=<string>` on `/quote` or `/prices`, without signing up for anything. **Pro API accounts** layer on top: higher RPS, larger daily quotas, an analytics dashboard, early feature access, and SLA-backed support for production workloads.
