...
- Use [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build) if you are building the HTTP flow directly. Submit the signed order through [`POST /delta/v2/orders`](/api-reference/delta/orders-submit) and track slice execution through the Delta order endpoints.
