...
- Building the HTTP flow directly? Build the order the user will sign with [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build), then submit it through [`POST /delta/v2/orders`](/api-reference/delta/orders-submit) and track its status from there.
