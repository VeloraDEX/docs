...
- **AugustusRFQ** — on-chain OTC for ERC-20, with optional partial fills.

You can pick a single surface or run them side by side. The Delta API exposes `GET /v2/quote`, which accepts a `mode` (`ALL`, `DELTA`, `MARKET`) and returns either a `delta` block or a `market` block. Velora picks the winner server-side. The Market API exposes `GET /prices` for direct aggregator routing. See [Trading modes](/integrate/trading-modes).
