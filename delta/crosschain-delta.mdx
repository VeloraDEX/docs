...
- Your integration already uses Delta orders and can pass the `delta.route` from the quote response through to `POST /delta/v2/orders/build` unchanged.
...
- Treat the returned `delta.route` as the source of truth. Pass it verbatim into `POST /delta/v2/orders/build`.
...
<Steps>
  <Step title="Build the order">
    Send the returned `delta.route` in `POST /delta/v2/orders/build`, with `owner` set to the user's address.
  </Step>
  ...
  <Step title="Submit and poll">
    Submit the order with `POST /delta/v2/orders`, then poll `GET /delta/v2/orders/{orderId}` until the status reaches a terminal state.
  </Step>
</Steps>
...
- [Delta quote endpoint](/api-reference/quote/quote) — quote parameters and response shape.
