...
Pass the unmodified `delta.route` into `POST /delta/v2/orders/build` (with `owner` set to the user address) to get EIP-712 typed data back as `{ toSign, orderHash }`. Sign `toSign` with an ERC-2098 compact signature, then `POST /delta/v2/orders` with `order` (the `toSign.value`), the `signature`, `chainId`, and your `partner`. Poll `GET /delta/v2/orders/{orderId}` for status. See [Delta → How it works](/delta/how-it-works).
