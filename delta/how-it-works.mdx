...
You call `GET /quote?mode=DELTA` — or `mode=ALL` to let Velora pick the path (see [Trading modes](/integrate/trading-modes)). The API returns a `delta` payload containing the indicative price, the expected `destAmount`, the deadline, the partner-and-fee header, and an `hmac` you must pass through verbatim in step 2.
