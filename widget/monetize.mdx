...
|---|---|---|
| `partner` | `string` | Your partner-key string (e.g. `"my-app-name"`). Used for attribution and analytics. |
...
<Note>
**Precedence.** If both `partnerFeeBps` and `partnerTakesSurplus` are set, `partnerFeeBps` wins. To take surplus instead of a fixed fee, set `partnerTakesSurplus: true` and **omit** `partnerFeeBps`.
</Note>
...
- **Never inline the partner key in source you publish unmodified.** Load `partner` from an environment variable so different builds (staging, production, white-label) can use different keys.
- **Memoize `partnerConfig`.** Like every non-primitive config field, a new object every render forces re-mount. Move it to a module-level constant or `useMemo`.
- **Audit the partner address.** Once a swap is routed, fees pay to `partnerAddress` on-chain. Treat it like any other treasury address.
