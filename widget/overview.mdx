...
The Velora Widget is a React component that gives your users a complete trading interface — swaps, limit orders, OTC, and TWAP — across every chain Velora supports. Drop `<Widget />` into your app, pass a `QueryClientProvider`, and you have a working DEX inside your product.
...
<CardGroup cols={2}>
  <Card title="Multi-chain swaps" icon="arrows-left-right">
    Ethereum, Polygon, BNB, Avalanche, Arbitrum, Optimism, Base, Gnosis, Unichain — same-chain and crosschain.
  </Card>
  ...
  <Card title="Partner fees" icon="coins">
    Take a fee or a surplus share on every trade. See [Monetize](/widget/monetize).
  </Card>
...
- **Quotes** come from the same [Market API](/api-reference/market/overview) and [Delta API](/api-reference/delta/overview) you'd hit from the SDK — no separate widget backend.
...
- **Styles** are auto-injected on import and scoped under `.velora-widget` (Tailwind v4) so they don't leak into your app.
