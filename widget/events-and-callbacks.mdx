...
<Warning>
**dApp-mode behavior.** When `widgetMode: "dapp"`:

- **Without** an `onConnectWalletClick` handler — the button is **disabled**.
- **With** an `onConnectWalletClick` handler — the button is **enabled**, but the widget will **not** open its own Connect Wallet drawer. You must run your own wallet connection flow inside the handler.
