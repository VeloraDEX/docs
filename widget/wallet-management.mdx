...
<Warning>
In `dapp` mode the widget's internal Connect Wallet drawer is **not** rendered. The behavior of the button depends on whether you pass an `onConnectWalletClick` handler:

- **Without** `onConnectWalletClick` — the button is **disabled**.
- **With** `onConnectWalletClick` — the button is **enabled**, but the widget will **not** open its own drawer. You must run your host app's wallet connection flow inside the handler.
</Warning>
