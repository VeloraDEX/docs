...
A `QueryClientProvider` from `@tanstack/react-query` must wrap `<Widget />` somewhere above it in the tree.
...
| Create React App | ✅ Works | No special setup beyond `QueryClientProvider`. |
...
Passing inline arrays or objects forces re-mount on every render. Memoize them.
