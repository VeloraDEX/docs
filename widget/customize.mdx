...
The widget exposes a single visual control today: a `theme` config field that switches between bundled light and dark themes. Colors, typography, and components are not currently configurable from the host app.

<Note>
**Scope, honestly.** There's no design-token theming API — you can't pass brand colors, fonts, or a border-radius scale yet. If that's a blocker for your integration, reach out to [support@velora.xyz](mailto:support@velora.xyz).
</Note>
