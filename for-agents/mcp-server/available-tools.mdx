...
- A Delta quote is completed through the Delta path (`POST /delta/v2/orders/build`, sign externally, `POST /delta/v2/orders`) — not through `velora_build_transaction`. The quote's `agentHints` repeat these steps.
