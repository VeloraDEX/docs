...
3. Complete the order outside the MCP server, exactly as the hints say:
   - Pass the quote's route **verbatim** to [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build), which returns unsigned EIP-712 typed data.
   - Have the user's wallet sign that typed data. The MCP server never signs.
   - Submit the signed order via [`POST /delta/v2/orders`](/api-reference/delta/orders-submit) and track it by order ID.
