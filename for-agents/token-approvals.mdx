...
- **Market.** Pass the signed `permit` or `permit2` payload into [`POST /transactions/:chainId`](/api-reference/market/transactions). Augustus v6.2 consumes it on the swap call — no separate approve transaction is needed.
