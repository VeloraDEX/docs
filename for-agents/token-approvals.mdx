...
- **Delta orders/build.** Pass the signed `permit` or `permit2` payload into [`POST /delta/v2/orders/build`](/api-reference/delta-v2/orders-build). If the token requires Permit/Permit2 and the payload is missing, the build call rejects with `400 Permit / Permit2 required` — see the [Delta orders/build failure modes](/api-reference/troubleshooting).
