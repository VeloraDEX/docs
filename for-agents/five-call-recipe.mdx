...
```text
1. GET /quote?mode=DELTA
2. POST /delta/orders/build
3. Ask the user to sign the EIP-712 typed data
4. POST /delta/orders
5. GET /delta/orders/:id until filled, failed, or expired
```
...
- Keep the top-level `delta` object and its matching `hmac` together.
- Pass the `delta` object forward unchanged.
- Do not grant approval to Augustus for a Delta order.
...
```text
1. GET /quote?mode=MARKET
2. POST /transactions/:network
...
<CardGroup cols={2}>
  ...
  <Card title="Error playbooks" icon="triangle-exclamation" href="/for-agents/reference/error-playbooks">
    Map failures to deterministic recovery actions.
  </Card>
</CardGroup>
