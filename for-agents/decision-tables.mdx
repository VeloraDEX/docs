...
description: "Deterministic routing rules for AI agents using Velora — map user intent, quote responses, wallet state, and errors to the next action."
...
|---|---|---|---|
| "Swap token A to token B" | `mode=ALL` | Call [`GET /quote`](/api-reference/delta/quote) with `mode=ALL`, `chainId`, token addresses, amount, slippage, and the app or project `partner` value. | Velora can choose the best available path. |
| "Gasless swap" or "I have no gas" | Delta | Call `GET /quote` with `mode=DELTA`. Continue only if the response includes a `delta` block. | Delta uses an EIP-712 signature and solver settlement. |
...
| "Protect me from MEV" | Delta | Call `GET /quote` with `mode=DELTA`. Continue only if the response includes a `delta` block. | MEV protection is a Delta-only scenario. |
| "Swap now" or "settle in this transaction" | Market API | Call `GET /quote` with `mode=MARKET`, then build a Market transaction. | Market returns calldata the user sends on-chain. |
...
|---|---|---|---|
| Top-level response field is `delta` | Continue the Delta order flow with [`POST /delta/orders/build`](/api-reference/delta/orders-build). | Pass the `delta` object and matching `hmac` from the same quote response. | Do not normalize, sort, round, or reconstruct the `delta` object. |
| Top-level response field is `market` | Continue the Market transaction flow with [`POST /transactions/:network`](/api-reference/market/transactions). | Pass the returned `market` object into the transaction build step. | Do not try to submit it as a Delta order. |
...
| Delta Swap | EIP-712 typed data | No | [`POST /delta/orders`](/api-reference/delta/orders) | Delta order ID and status |
| Delta `LIMIT` order | EIP-712 typed data from `POST /delta/v2/orders/build` | No at order creation | `POST /delta/v2/orders` with `order=toSign.value`, `signature`, `type=LIMIT`, `chainId`, and `partner` | Order ID and fill status |
| Delta TWAP | EIP-712 typed data from `POST /delta/v2/orders/build` | No at order creation | `POST /delta/v2/orders` after signing the built `TWAPOrder` or `TWAPBuyOrder` payload | Slice execution and order status |
...
|---|---|---|
| Limit order | `GET /delta/v2/prices` → `POST /delta/v2/orders/build` → user signs `toSign` → `POST /delta/v2/orders` with `type=LIMIT`. | Pass `route` and `toSign.value` verbatim. Use `limitAmount` when the target-price constraint needs an explicit amount override. |
| Sell TWAP | `GET /delta/v2/prices` for one source slice → `POST /delta/v2/orders/build` with `orderType=TWAPOrder`, `interval`, `numSlices`, and `totalSrcAmount`. | Preserve total source amount and split source across slices. `route.origin.input.amount` must equal `floor(totalSrcAmount / numSlices)`. |
| Buy TWAP | `GET /delta/v2/prices` for one destination slice → `POST /delta/v2/orders/build` with `orderType=TWAPBuyOrder`, `interval`, `numSlices`, `totalDestAmount`, and `maxSrcAmount`. | Preserve total destination amount, split destination across slices, and cap source spend. `route.origin.output.amount` must equal `floor(totalDestAmount / numSlices)`. |
| Read status | `GET /delta/v2/orders/{orderId}`, `GET /delta/v2/orders/hash/{hash}`, or paginated `GET /delta/v2/orders`. | Narrow on `type` and `onChainOrderType`: `LIMIT`, `TWAPOrder`, `TWAPBuyOrder`, `FillableOrder`, etc. |
...
<CardGroup cols={2}>
  ...
  <Card title="Limit orders" icon="chart-line" href="/overview/trading-concepts/limit-orders">
    Route target-price orders to Delta order flows.
  </Card>
  <Card title="TWAP" icon="clock" href="/overview/trading-concepts/twap">
