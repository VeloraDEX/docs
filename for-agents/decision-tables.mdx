...
|---|---|---|---|
| Top-level response field is `delta` | Continue the Delta order flow with [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build). | Pass the `delta` object and matching `hmac` from the same quote response. | Do not normalize, sort, round, or reconstruct the `delta` object. |
...
| Delta Swap | EIP-712 typed data | No | [`POST /delta/v2/orders`](/api-reference/delta/orders-submit) | Delta order ID and status |
