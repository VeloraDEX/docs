...
|---|---|---|---|
| "Swap token A to token B" | `mode=ALL` | Call [`GET /quote`](/api-reference/delta-v2/quote) with `mode=ALL`, `chainId`, token addresses, amount, slippage, and the app or project `partner` value. | Velora can choose the best available path. |
...
|---|---|---|---|
| Top-level response field is `delta` | Continue the Delta order flow with [`POST /delta/v2/orders/build`](/api-reference/delta-v2/orders-build). | Pass the `delta` object and matching `hmac` from the same quote response. | Do not normalize, sort, round, or reconstruct the `delta` object. |
...
| Delta Swap | EIP-712 typed data | No | [`POST /delta/v2/orders`](/api-reference/delta-v2/orders-submit) | Delta order ID and status |
...
<CardGroup cols={2}>
  ...
  <Card title="Delta quote" icon="code" href="/api-reference/delta-v2/quote">
