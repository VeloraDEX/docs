...
Requirements:
- Velora endpoint: `GET https://api.velora.xyz/quote` with `mode=ALL` and `partner=my-app-name` (replace with the host app's partner-key string). The `mode=ALL` response contains exactly one of `delta` or `market` (server-picked winner). To compare Delta vs Market head-to-head before competing against the other aggregator, issue two requests in parallel (`mode=DELTA` and `mode=MARKET`), then pick the better Velora result.
