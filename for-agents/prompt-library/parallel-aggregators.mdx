...
Requirements:
- Velora endpoint: `GET https://api.velora.xyz/quote` with `mode=ALL` and `partner=my-app-name` (replace with the host app's partner-key string). The response may contain `market` (Market route) and/or `delta` (Delta intent) — pick the better one within the Velora result, then compete it against the other aggregator's quote.
