...
<CardGroup cols={2}>
  ...
  <Card title="Use agent tools" icon="plug" href="/for-agents/mcp-server/overview">
    Connect MCP clients or CLI workflows when you want tools instead of raw HTTP.
  </Card>
</CardGroup>
...
- Use [Limit orders](/overview/product-stack/limit-orders) and [TWAP](/overview/product-stack/twap) when the user wants delayed execution, target prices, or time-sliced trades. For API-based agents, build these flows through `/delta/v2/prices`, `/delta/v2/orders/build`, `/delta/v2/orders`, and Delta order read endpoints.
