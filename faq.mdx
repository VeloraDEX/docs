...
For Delta, slippage is applied at the build step. Pass `slippage` to [`POST /v2/delta/orders/build`](/api-reference/delta/orders-build); the API returns an order with `destAmount` reduced by that slippage. You then sign and submit via [`POST /v2/delta/orders`](/api-reference/delta/orders-submit). The signed `order.destAmount` is the limit-order floor: solvers must deliver at least that amount or the fill doesn't settle.
...
Delta doesn't support tax or fee-on-transfer tokens today; route them through Market instead. On [`GET /prices`](/api-reference/market/prices), declare the fees so the quote accounts for them:
...
Market only. Pass `includeDEXS` or `excludeDEXS` to [`GET /prices`](/api-reference/market/prices) as a comma-separated list of DEX names and the router will restrict the route to (or exclude) those venues. Delta has no equivalent: solvers pick their own execution path.
...
### Does a failed Delta order cost anything?

No. Whether the order expires, is rejected, or settlement reverts on-chain, you pay no gas and your funds stay in your wallet. On Market, by contrast, a reverted swap still costs you the gas of the failed call. Side-by-side breakdown: [Trading modes](/integrate/trading-modes).
