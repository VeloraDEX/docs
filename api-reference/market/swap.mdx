...
<Steps>
  <Step title="Approve (ERC-20 source only)">
    Approve the Augustus v6.2 router (`txParams.to`) to spend `srcToken` — a standard `approve(to, amount)`. Native sources (ETH, etc.) skip this; the amount rides along in `txParams.value`. To drop the extra transaction, switch to the two-step flow and pass Permit / Permit2 data in the build call.
  </Step>
