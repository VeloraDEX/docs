...
- **Unified quote.** `GET /quote` returns either a `delta` block or a `market` block (or both with `mode=ALL`) — useful when you want Delta with automatic Market fallback. V2 offers the same via [`GET /v2/quote`](/api-reference/quote/v2-quote).
...
This V1 HTTP surface has no SDK wrapper. For the SDK's Delta methods (`sdk.delta.*`) and a 5-minute walkthrough, see [SDK → Delta](/sdk/products/delta).
