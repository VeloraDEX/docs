...
Run with `PROVIDER_URL=https://mainnet... npx hardhat test`. The Velora SDK exposes overrides for impersonation via your provider — see the SDK's [own test setup](https://github.com/VeloraDEX/sdk/tree/main/tests) for a reference.
...
const rate = await sdk.swap.getRate({
  /* fill in srcToken/destToken/amount/userAddress */
});
