...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| `sdk.deltaV2`     | **Recommended.** `getDeltaPrice`, `buildDeltaOrder`, `signDeltaOrder`, `postDeltaOrder`, `submitDeltaOrder`, `approveTokenForDelta`, `preSignDeltaOrder`, `getDeltaContract`, `getDeltaOrderById`, `getDeltaOrderByHash`, `getDeltaOrders`, `cancelDeltaOrders`, `getBridgeRoutes`, `isTokenSupportedInDelta` | [Delta](/sdk/products/delta)                          |
| `sdk.delta`       | Delta V1. Same lifecycle, locally-built orders. See [Delta V1](/sdk/products/delta#delta-v1).                                                                                                                                                                         | [Delta](/sdk/products/delta#delta-v1)                 |
...
| `sdk.otc`         | AugustusRFQ maker/taker orders for OTC trades. For target-price limit orders, use the Delta namespace instead.                                                                                                                                                       | [OTC](/sdk/products/otc)                              |
...
if ("delta" in quote) {
  ...
  const destAmount = BigInt(
    +(+quote.delta.destAmount * (1 - slippageBps / 10000)).toFixed(0),
  ).toString(10);
...
See [Swaps → Delta](/sdk/products/delta) for the split build/sign/post flow and order-status polling (including the recommended Delta V2 path), and [Swaps → Market](/sdk/products/swap) for the full Market-Swap walkthrough.
