...
A **swap** in Velora settles through one of two execution paths: [**Market**](/sdk/products/swap) or **Delta** (this page). Delta wraps the [Delta API](/delta/overview) — gasless, MEV-protected swaps where the user signs an off-chain order and a network of solvers (Portikus) competes to fill it. The user pays no gas; the solver covers execution cost out of their margin.

Delta is also the engine behind two other products in the SDK — [Limit orders](/sdk/products/limit-orders) (a Delta order with a target price) and [TWAP](/sdk/products/twap) (a Delta order split into scheduled slices). This page covers the **market** Delta order; those pages build on the same flow.
...
- **[TWAP](/sdk/products/twap)** — a Delta order split into time-weighted slices, built with `buildTWAPDeltaOrder` / `submitTWAPDeltaOrder`.
