---
title: "Delta"
description: "Submit, sign, and track gasless Delta intents from sdk.deltaV2, where solvers execute swaps on the user's behalf."
...
- **`sdk.deltaV2.*`** is the pre-bound bag attached to `constructSimpleSDK` / `constructFullSDK`. Methods on it are **unsuffixed**: `sdk.deltaV2.getDeltaPrice`, `sdk.deltaV2.submitDeltaOrder`, `sdk.deltaV2.signDeltaOrder`, etc. This is what most consumers use.
...
`import type { DeltaV2 }` works the same way for type-only consumers. Types live *inside* the namespace, so alias with `type DeltaPrice = DeltaV2.DeltaPrice;` if you want a short local name. You can't destructure a type at the import site.
...
`buildDeltaOrder` returns `{ toSign, orderHash }`. The server has already composed the EIP-712 typed data, and `signDeltaOrder` wraps your contract caller's `signTypedData` to produce the signature.
...
Each entry is a `DeltaV2.DeltaAuction<T>`, generic over `onChainOrderType` (the same shape as V1's `DeltaAuction`). It may report `'FillableOrder'` for a `partiallyFillable` Standard order; treat it exactly like `'Order'` (identical shape). Use `type: "LIMIT"` to filter for [limit orders](/sdk/products/limit-orders).

## Partner fee

Pass `partner` (and optionally `partnerAddress`, `partnerFeeBps`, `partnerTakesSurplus`) to `getDeltaPrice`, `submitDeltaOrder`, `buildDeltaOrder`, and `postDeltaOrder`. V2 sends these directly to the server, which resolves and validates the fee in one call, so there's no local `getPartnerFee` round-trip.
...
V2 method names are the same as V1's at the property level (`buildDeltaOrder`, `signDeltaOrder`, `postDeltaOrder`). What changes is the SDK namespace (`sdk.delta` → `sdk.deltaV2`) and the param/response shapes. `buildDeltaOrder` returns `{ toSign, orderHash }` from the server. `signDeltaOrder` is generic across order families, so the market, limit, and TWAP flows all sign through the same call.
...
- `sdk.deltaV2.getDeltaContract()` returns the same contract address.
- `sdk.deltaV2.getPartnerFee({ partner })` is useful for showing fee info in your UI. V2 doesn't *need* this round-trip because the server resolves partner fees, but the helper is still there.
...
const auction = await sdk.delta.submitDeltaOrder({
  ...
  srcAmount: amount,
  destAmount: BigInt(
    +(+deltaPrice.destAmount * (1 - 50 / 10000)).toFixed(0),
  ).toString(10), // 0.5% slippage
  partner: "my-app-name",
});
```
