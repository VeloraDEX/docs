...
Each entry is a `DeltaV2.DeltaOrderResponse`. The list may include a fifth `onChainOrderType` value — `'ProductiveOrder'` — that has no SDK builder; see [Productive orders](#productive-orders-read-only) below.
...
- **External Delta orders** — settle through a custom handler contract. Used for Aave-style collateral/debt swaps and other DeFi compositions. Use `buildExternalDeltaOrder` / `submitExternalDeltaOrder`. See [`EXTERNAL_ORDERS.md`](https://github.com/VeloraDEX/sdk/blob/main/docs/EXTERNAL_ORDERS.md) in the SDK repo.
- **TWAP Delta orders** — split a large order into time-weighted chunks (sell-side or buy-side). Use `buildTWAPDeltaOrder` / `submitTWAPDeltaOrder`. See [`DELTA.md`](https://github.com/VeloraDEX/sdk/blob/main/docs/DELTA.md) in the SDK repo.
...
V2 returns a `PaginatedResponse<DeltaV2.DeltaOrderResponse>` envelope. `userAddress` is required; iterate `data` and increment `page` until you've consumed `total`.
...
+ import { DeltaV2 } from "@velora-dex/sdk";
+ // then: DeltaV2.DeltaPrice, DeltaV2.BuiltDeltaOrder, DeltaV2.DeltaOrderResponse
```
