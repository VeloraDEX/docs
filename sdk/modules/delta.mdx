...
keywords: ["sdk", "delta", "deltaV2", "intents", "gasless", "submitDeltaOrderV2"]
---
...
<Steps>
  <Step title="Get a Delta price">
    Call `sdk.deltaV2.getDeltaPriceV2` with source/destination tokens, amount, decimals, and partner. The response includes the recommended `route` plus `alternatives` and the contract `spender` to approve.
  </Step>
  ...
  <Step title="Build, sign, and post the order">
    Either call the one-shot orchestrator `sdk.deltaV2.submitDeltaOrderV2`, or run the three steps yourself (`buildDeltaOrderV2` → `signDeltaOrderV2` → `postDeltaOrderV2`) when you need control over signing.
  </Step>
  <Step title="Poll for execution">
    Use `sdk.deltaV2.getDeltaOrderByIdV2` (or `getDeltaOrderByHashV2`) on a timer until status is `COMPLETED`.
  </Step>
</Steps>

## One-call submit

`submitDeltaOrderV2` runs build → sign → post in a single call. This is the recommended path for most flows.

```ts
const price = await sdk.deltaV2.getDeltaPriceV2({
  srcToken: DAI,
  destToken: USDC,
...
```ts
const built = await sdk.deltaV2.buildDeltaOrderV2({
  route: price.route,
  side: price.side,
  owner: account,
  partner: "my-app-name",
...
const signature = await sdk.deltaV2.signDeltaOrderV2(built);

const auction = await sdk.deltaV2.postDeltaOrderV2({
...
`buildDeltaOrderV2` returns `{ toSign, orderHash }` — the server has already composed the EIP-712 typed data; `signDeltaOrderV2` wraps your contract caller's `signTypedData` to produce the signature.
...
const intervalId = setInterval(async () => {
  const updated = await sdk.deltaV2.getDeltaOrderByIdV2(auction.id);
...
`getDeltaOrdersV2` returns a paginated list filtered by user, chain, status, and order type:

```ts
const { data, page, limit, total } = await sdk.deltaV2.getDeltaOrdersV2({
  userAddress: account,
  page: 1,
  limit: 100,
...
Pass `partner` (and optionally `partnerAddress`, `partnerFeeBps`, `partnerTakesSurplus`) to `getDeltaPriceV2`, `submitDeltaOrderV2`, `buildDeltaOrderV2`, and `postDeltaOrderV2`. V2 sends these directly to the server, which resolves and validates the fee — no local `getPartnerFee` round-trip needed.
...
```ts
await sdk.deltaV2.cancelDeltaOrdersV2({ orderIds: [auction.id] });
```
...
Pass `destChainId` to `getDeltaPriceV2` to settle on a different chain than the source. The returned `route` is bridge-aware; submitting the order is identical to same-chain. See [Crosschain Delta](/delta/crosschain-delta) for the full design and [`getBridgeRoutes`](https://github.com/VeloraDEX/sdk/blob/main/src/methods/deltaV2/getBridgeRoutes.ts) for advanced route inspection.

## External and TWAP orders

The SDK supports two advanced order families on V2 that share the same build/sign/post pattern:

- **External Delta orders** — settle through a custom handler contract. Used for Aave-style collateral/debt swaps and other DeFi compositions. Use `buildExternalDeltaOrderV2` / `submitExternalDeltaOrderV2`. See [`EXTERNAL_ORDERS.md`](https://github.com/VeloraDEX/sdk/blob/main/docs/EXTERNAL_ORDERS.md) in the SDK repo.
- **TWAP Delta orders** — split a large order into time-weighted chunks (sell-side or buy-side). Use `buildTWAPDeltaOrderV2` / `submitTWAPDeltaOrderV2`. See [`DELTA.md`](https://github.com/VeloraDEX/sdk/blob/main/docs/DELTA.md) in the SDK repo.

Both reuse the generic `signDeltaOrderV2` and the V2 polling endpoints.
...
V1 returns `DeltaPrice { destAmount, partner, partnerFee, srcAmount, destToken }`. V2 returns `DeltaPriceV2 { route, alternatives, partner: { name, feePercent }, spender, ... }` — swap details live under `price.route`, and `alternatives` lets you show users different bridges.
...
`buildDeltaOrderV2` returns `{ toSign, orderHash }` from the server. `signDeltaOrderV2` is generic across all order families (Order, ExternalOrder, TWAPOrder, TWAPBuyOrder) — V1's separate `signExternalDeltaOrder` / `signTWAPDeltaOrder` collapse into one call.
...
V2 returns a `PaginatedResponse<DeltaOrderV2Response>` envelope. `userAddress` is required; iterate `data` and increment `page` until you've consumed `total`.

### Cancelling

```diff
- await sdk.delta.cancelLimitDeltaOrders([orderId]);
+ await sdk.deltaV2.cancelDeltaOrdersV2({ orderIds: [orderId] });
```
