---
title: "Monetize"
description: "Add partner fees or surplus share to every SDK call. The same partner key works across Delta orders, Market Swaps, and quotes."
...
<Note>
**Delta V2 vs V1.** V2 sends `partner` / `partnerAddress` / `partnerFeeBps` straight to the server, which resolves and validates them in one call. V1 calls `getPartnerFee(partner)` locally inside `buildDeltaOrder` (response is cached per partner). Either way the fee lands on-chain at settlement; the V2 path is just one fewer network call.
</Note>
