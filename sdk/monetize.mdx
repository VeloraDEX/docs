...
- **Delta V2** (recommended) — pass `partner` (and overrides) to `sdk.deltaV2.getDeltaPriceV2`, `sdk.deltaV2.submitDeltaOrderV2`, `sdk.deltaV2.buildDeltaOrderV2`, and `sdk.deltaV2.postDeltaOrderV2`. The server encodes the fee into the on-chain order.
