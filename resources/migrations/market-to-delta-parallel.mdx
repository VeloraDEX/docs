...
- **No code rewrite** — `GET /v2/quote` with `mode=ALL` returns either a Market `market` block (same shape as `/prices`'s `priceRoute`) or a Delta `delta` block — Velora picks the winner server-side and you branch on which block is present. See [Trading modes](/integrate/trading-modes).
...
3. For the Delta path, call `POST /delta/v2/orders/build`, sign the EIP-712 typed data, then `POST /delta/v2/orders`.
