...
- Pricing is route-based. `GET /v2/delta/prices` returns a recommended `route` plus `alternatives`, so users can pick a different bridge and you stop juggling the `bridge` / `bridgeInfo` / `availableBridges` triple V1 returns.
...
1. `GET /v2/delta/prices` returns a `route` and a non-empty `spender`.
