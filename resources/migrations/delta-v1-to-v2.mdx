...
- Status is a single field. Eleven values (`PENDING`, `AWAITING_SIGNATURE`, `ACTIVE`, `SUSPENDED`, `CANCELLING`, `BRIDGING`, `COMPLETED`, `FAILED`, `EXPIRED`, `CANCELLED`, `REFUNDED`) replace V1's `status` + `bridgeStatus` pair.
...
| `POST /delta/orders/cancel` | [`POST /v2/delta/orders/cancel`](/api-reference/delta/orders-cancel) | Same shape |
...
- The status enum changed, so your status-handling code needs the mapping table above. Don't rely on `EXECUTED` alone: V2 reports `COMPLETED` for executed-and-finalized orders and `BRIDGING` while the destination leg is still pending.
...
4. `GET /v2/delta/orders/{id}` polls through `ACTIVE` → `COMPLETED` (or `BRIDGING` → `COMPLETED` for crosschain).
