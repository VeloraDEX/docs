...
A **limit order** on Velora is a [Delta order](/integrate/api/delta-swap) with a target-price constraint: the user signs once, the [Portikus Network](/solver-network/portikus) fills it at or better than the limit price when the market allows, and the user pays no gas. This page walks the whole integration over plain HTTP. It's the Delta swap flow with two additions: you set `limitAmount` at build, and submit with `type: "LIMIT"`. Every example targets `https://api.velora.xyz`.
...
Pass the `delta.route` verbatim and add `limitAmount`. For a SELL order it's the minimum destination amount you'll accept (your limit price); a `deadline` is strongly recommended, or the order rests until cancelled.
