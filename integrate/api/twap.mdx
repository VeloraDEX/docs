...
<Steps>
  ...
  <Step title="Build the schedule">
    `POST /delta/v2/orders/build` with the `orderType`, `interval`, `numSlices`, a `deadline`, and the totals. See [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build).
  </Step>
  <Step title="Sign and submit">
    The user signs `toSign`; `POST /delta/v2/orders`. See [`POST /delta/v2/orders`](/api-reference/delta/orders-submit).
  </Step>
...
Poll `GET /delta/v2/orders/{orderId}` over the order's lifetime; the `input`/`output` amounts carry expected and executed totals, so you can show fill progress without computing it yourself. For a human-readable view of slice-by-slice progress, open the order in the Velora explorer at `https://explorer.velora.xyz/order/{orderId}`. List a user's TWAPs by filtering on `onChainOrderType`:
