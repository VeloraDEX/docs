...
A buy TWAP fixes how much to receive and caps what's spent. Quote a single slice by the per-slice destination amount, then build with `totalDestAmount` and `maxSrcAmount`. Each slice may spend at most its proportional share of `maxSrcAmount`, so a single expensive slice can't eat the whole budget:
...
Cancel the remaining (unsettled) slices with a gasless signed cancellation. Build the cancellation payload first, have the user sign the returned `toSign` object, then post the same order IDs with the signature:
