...
<Steps>
  ...
  <Step title="Build the order">
    `POST /delta/v2/orders/build` with the `delta.route` passed **verbatim**, `owner` set to the user, and a `deadline`. You get back `{ toSign, orderHash }`. See [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build).
  </Step>
