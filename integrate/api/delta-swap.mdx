...
Poll until the order reaches a terminal status. A same-chain market Delta order settles in seconds once a solver wins the auction, so a short interval is fine. Crosschain orders can spend longer in `BRIDGING`, or in `REFUNDING` if the bridge leg failed and a refund transaction is still being verified.
