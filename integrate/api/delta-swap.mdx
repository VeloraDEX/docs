---
title: "Delta swap"
description: "Integrate a gasless, MEV-protected Delta swap over the REST API: quote, build, sign, submit, and poll an intent end-to-end with plain HTTP."
keywords: ["delta api", "gasless swap", "intent", "quote", "orders build", "orders submit", "dETH", "native eth"]
---

A **Delta swap** lets your user sign one off-chain intent and have the [Portikus Network](/solver-network/portikus) settle it gaslessly, with MEV protection. This page walks the whole integration over plain HTTP, end to end: quote the trade, build the order, have the user sign it, submit it, and poll until it settles. Every example is a `curl` or `fetch` call against `https://api.velora.xyz`; no SDK required.
...
<Steps>
  ...
  <Step title="Build the order">
    `POST /delta/v2/orders/build` with the `delta.route` passed **verbatim** and `owner` set to the user. You get back `{ toSign, orderHash }`. See [`POST /delta/v2/orders/build`](/api-reference/delta/orders-build).
  </Step>
  <Step title="Sign and submit">
    The user signs `toSign` (ERC-2098 compact signature). `POST /delta/v2/orders` with the `order`, `signature`, `chainId`, and `partner`. See [`POST /delta/v2/orders`](/api-reference/delta/orders-submit).
  </Step>
  <Step title="Poll until settled">
    `GET /delta/v2/orders/{orderId}` until the status is terminal (`COMPLETED`, `EXPIRED`, `FAILED`, `CANCELLED`). See [`GET /delta/v2/orders/{orderId}`](/api-reference/delta/orders-get-by-id).
  </Step>
</Steps>
...
Pass the `delta.route` from the quote **verbatim** into the build call, with `owner` set to the user's address:
