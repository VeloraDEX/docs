...
<Steps>
  ...
  <Step title="Poll until settled">
    `GET /v2/delta/orders/{orderId}` until the status is terminal (`COMPLETED`, `EXPIRED`, `FAILED`, `CANCELLED`). See [`GET /v2/delta/orders/{orderId}`](/api-reference/delta/orders-get-by-id).
  </Step>
</Steps>
