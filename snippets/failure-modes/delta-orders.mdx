...
| 200 OK, but order stays at `pending` indefinitely        | Solver auction returned no fill (size, pair, or chain conditions)               | Poll `GET /delta/orders/:id`; if still pending past your SLA, cancel and re-quote — see [Order lifecycle & status codes](/delta/order-lifecycle-and-status-codes) |
