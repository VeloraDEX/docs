...
| 200 OK then stuck at `PENDING`                                    | No agent bid the auction (size, pair, or chain) within the window                | Poll [GET /v2/delta/orders/{orderId}](/api-reference/delta/orders-get-by-id); build a cancellation payload with [POST /v2/delta/orders/build/cancellation](/api-reference/delta/orders-build-cancellation), then cancel and re-quote if past your SLA |
