...
|---|---|---|
| 400 `No route` for a same-chain pair                          | Token pair has no liquidity, or `srcDecimals` / `destDecimals` are wrong              | Re-check decimals against the token contract; try `mode=ALL` on [GET /quote](/api-reference/delta-v2/quote) to see whether Market can route it |
