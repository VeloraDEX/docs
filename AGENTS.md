...
- **`developers.velora.xyz` is deprecated.** It is being migrated to Mintlify; all canonical Velora developer documentation will live under `new-docs.velora.xyz`. Don't link to `developers.velora.xyz` from new content, and don't write it into source-of-truth fields (e.g. `_meta.sources` in JSON companions, `externalDocs.url` in OpenAPI specs). When pulling data from a `developers.velora.xyz` page during a one-time fetch, copy the data into the repo as the new source of truth — don't leave the URL as a back-reference.
