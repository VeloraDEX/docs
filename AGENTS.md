...
- **Failure-mode standard for API reference pages**: every endpoint page in `/api-reference/` must end with a `## Troubleshooting` section (5–8 rows: symptom → root cause → fix) placed at the **bottom** of the page, after Response. Tables live as MDX snippets under `/snippets/failure-modes/<endpoint-slug>.mdx` and are imported at the top of the page (`import X from "/snippets/failure-modes/<endpoint-slug>.mdx";`), then rendered as `<X />` under the H2. To make this ordering possible, these pages do **not** use `openapi:` frontmatter (which forces all body MDX above the auto-rendered API block); instead they render params and response inline with `<ParamField>` and `<ResponseField>` components. The frontmatter uses an `api: "METHOD https://api.velora.xyz/path"` field so the "Try it" panel still renders.
